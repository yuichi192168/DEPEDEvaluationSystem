...
// Always insert a new draft to allow multiple drafts to be saved
// Generate unique identifier combining timestamp and applicant name
$timestamp = date('YmdHis');
$applicantName = isset($data['applicant_name']) ? $conn->real_escape_string($data['applicant_name']) : 'Unknown';
$uniqueCode = $appCode ? $appCode . '_' . $timestamp : null;

// Insert new draft
$ins = "INSERT INTO drafts (session_id, application_code, data) VALUES ('{$session_id}', " . ($uniqueCode ? "'{$uniqueCode}'" : "NULL") . ", '{$json}')";
$ok = $conn->query($ins);
if ($ok) {
